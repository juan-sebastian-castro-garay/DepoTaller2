...
public class SandboxConjuntos
{
    ...
    {
    	List<String> listaCadenasInvertida = new LinkedList<>();

        for (String cadena : arbolCadenas.descendingSet()) {
            listaCadenasInvertida.add(cadena);
        }

        return listaCadenasInvertida;
    }
    ...
    {
    	TreeSet<String> arbolInvertido = new TreeSet<>((a, b) -> b.compareTo(a));
    ...
    {
    	boolean todosEnConjunto = true;
    	for (String cadena : otroArreglo) {
    	    if (!arbolCadenas.contains(cadena)) {
    	        todosEnConjunto = false;
    	        break;
    	    }
    	}
    	return todosEnConjunto;
